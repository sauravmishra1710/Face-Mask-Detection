...
class FACE_MASK_TYPE(Enum):
    
    '''
    An ENUM class to contain the enumeration values for masked / un-masked faces.
    '''
    
    MASKED = 'with_mask'
    NON_MASKED = 'without_mask'
...
class dataUtils():
    ...
    def GetLabelledFaceMaskData(self):
        ...
        Return Value: 
            The computed face dataframe.
        '''
        
        masked_images = glob.glob('facial-images/with_mask/*.*')
        non_masked_images = glob.glob('facial-images/without_mask/*.*')
        len(masked_images), len(non_masked_images)

        face_df = pd.DataFrame({
            'filename': masked_images + non_masked_images,
            'label': ['Masked'] * len(masked_images) + ['UnMasked'] * len(non_masked_images)
        })

        # Shuffle the rows in the dataset
        face_df = face_df.sample(frac=1, random_state=34).reset_index(drop=True)
    ...
    def GetImageDirectory(self, imageCategory):
        ...
        '''
        Purpose: 
            Gets the directory path for the category of image (Masked/Un-Masked) in the dataset.

        Parameters:
            1. imgCategory - The category of the image (Masked/Un-Masked). See RBC_CELL_TYPE ENUM.
        ...
        if imageCategory == FACE_MASK_TYPE.MASKED.value:
            imgDirectory = 'facial-images/with_mask/*.*'
        else:
            imgDirectory = 'facial-images/without_mask/*.*'
    ...
    def GetAllImageShape(self, imageCategory):
        ...
        Purpose: 
            ...
            (Masked/UnMasked). As the number of images are large, this method utilizes parallel 
            processing using the ThreadPoolExecutor for faster computation.

        Parameters:
            1. imgCategory - The category of the image (Masked/UnMasked). See RBC_CELL_TYPE ENUM.
    ...
    def ReadAndDisplayInputImages(self, imageCategory, numImagesToDisplay):
        ...
        '''
        Purpose: 
            Read and display the first 5 images of the imageCategory (MASKED/NON_MASKED) 
            in the dataset.

        Parameters:
            1. imgCategory - The category of the image (MASKED/NON_MASKED). See FACE_MASK_TYPE ENUM.
    ...
    def GetSampleImage(self, imgCategory):
        ...
        '''
        Purpose:
            Reads and returns the first image of the given category - (MASKED/NON_MASKED).
    ...
    def GetRandomImageName(self, imgCategory):
        ...
        '''
        Purpose: 
            Gets a random image of the given category - (MASKED/NON_MASKED).
        ...
        directoryPath ='facial-images/' + imgCategory + '/'
